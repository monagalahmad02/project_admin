  ...
  Future<void> getAllHalls() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData is List) {
          hallsList.value =
              jsonData.map((hall) => HallPending.fromJson(hall)).toList();
          print('Parsed halls count: ${hallsList.length}');

          if (hallsList.isNotEmpty && hallsList.first.owner != null) {
            selectedOwnerId.value = hallsList.first.owner?.id;
            print('Selected owner ID: ${selectedOwnerId.value}');
          } else {
            print('Owner is null in the first hall');
          }
        } else if (jsonData is Map && jsonData.containsKey('data')) {
          var dataList = jsonData['data'] as List;
          hallsList.value =
              dataList.map((hall) => HallPending.fromJson(hall)).toList();
          print('Parsed halls count: ${hallsList.length}');
